...
3. 服务器遵循 HTT P协议格式，组织 HTML 文档作为 HTTP 响应的 Body 发送给浏览器；
...
于是就出现了 WSGI，需要清楚的是WSGI 不是服务器，也不是用于与程序交互的 API，更不是真实的代码，而只是定义的一个接口。目标是在 Web 服务器和 Web 框架层之间提供一个通用的 API 标准，减少之间的互操作性并形成统一的调用方式。
